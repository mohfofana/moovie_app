...
const MoviesSlides: FC<MoviesSlidesProps> = ({ movies, category }) => {
  ...
  const pushOffset = 180; // expanded width (380) - base width (200)

  return (
    <Swiper slidesPerView="auto" spaceBetween={16} className="mySwiper !overflow-visible">
      {movies.map((movie, index) => {
        ...
        return (
          <SwiperSlide
            key={movie.id}
            className="flex mt-1 flex-col xs:gap-[14px] gap-2 !overflow-visible rounded-lg !w-[200px]"
            style={{
